...
//        $auth->addChild($admin, $commentAuditor);
        ...
        // 添加 "adminuserPrivilege" 权限
        $adminuserPrivilege = $auth->createPermission('adminuserPrivilege');
        $adminuserPrivilege->description = '授予权限';
        $auth->add($adminuserPrivilege);

        // 添加 "adminuser" 角色并赋予 "updatadminuserPrivilegeePost"
        $adminuser = $auth->createRole('adminuser');
        $adminuser->description = '用户管理员';
        $auth->add($adminuser);
        $auth->addChild($adminuser, $adminuserPrivilege);
        $auth->assign($adminuser, 1);
